...
def hash_password(password: str) -> str:
    return ph.hash(password)  # turn a plain pw to a secure hashed pw


def verify_password(plain_password: str, hashed_password: str) -> bool:
    try:
        # check if the login password matches the stored hash
...
def create_access_token(data: dict, expires_delta: timedelta | None = None) -> str:
    # copy the data dic so we don't modify the original, data = sub: trevor
    to_encode = data.copy()
...
def get_current_user(
        # fastapi gets the token from auth header
        token: str = Depends(oauth2_scheme),
        # database session dependency, gives access to db
    ...
    try:
        ...
        # Extract the "sub" field from the token payload
