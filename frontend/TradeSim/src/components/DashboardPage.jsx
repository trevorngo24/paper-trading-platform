...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Step 5 — no token, kick to login
    if (!token) {
      navigate("/login");
      return;
    }

    // Step 6 — verify token with backend
    fetch(API_BASE + "/me", {
      ...
      .then((data) => {
        if (data) setUser(data); // Step 7 — store user info
      });
  }, []);
  ...
  // Step 7 — render protected content
  return (
    <div className="min-h-screen relative">
      <PixelSky />
      <Navbar />
